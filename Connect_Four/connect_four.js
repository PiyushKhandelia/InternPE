...
const getRightDiagonal = (
  ...
  while (rowCount > 0) {
    ...
    rowCount -= 1;
    columnCount += 1;
    rightDiagonal.unshift(
      initialMatrix[rowCount][columnCount]
    );
  }
  rowCount = row;
  columnCount = column;
...
//Check diagonal
const checkAdjacentDiagonalValues = (
  row,
  column
) => {
...
const setPiece = (startCount, colValue) => {
  ...
  if (initialMatrix[startCount][colValue] != 0) {
    ...
    //Update Matrix
    initialMatrix[startCount][colValue] =
      currentPlayer;
    //Check for wins
...
const fillBox = (e) => {
  ...
  playerTurn.innerHTML = `Player <span>${currentPlayer === 1
    ? player1Name
    : player2Name
    }'s</span> turn`;
};
...
const matrixCreator = () => {
  for (let innerArray in initialMatrix) {
    ...
    for (let j in initialMatrix[innerArray]) {
      ...
      let innerDiv = document.createElement(
        "div"
      );
